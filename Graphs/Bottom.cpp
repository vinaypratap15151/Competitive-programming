...
// its elements being called vertices (or nodes). Let E be a subset of the Cartesian product V×V,
// its elements being called edges. Then G=(V,E) is called a directed graph.
// Let n be a positive integer, and let p=(e1,…,en) be a sequence of length n of edges
// ei∈E such that ei=(vi,vi+1)for a sequence of vertices (v1,…,vn+1).
//     Then p is called a path from vertex v1 to vertex vn+1 in G and 
//     we say that vn+1 is reachable from v1, writing (v1→vn+1).
