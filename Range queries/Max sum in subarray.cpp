...
void lazy_update_setter(tree_attrs* tree, lazy_attrs* lazy, int start, int end, int left, int right, int current_position, int x)
{
    ...
    if(lazy[current_position].setter!=0){
        tree[current_position].sum_of_squares=(right-left+1)*lazy[cuurent_position].setter*lazy[cuurent_position].setter;
...
tree[current_position].sum=tree[2*treenode].sum + tree[2*treenode+1].sum;
 tree[current_position].sum_of_squares=tree[2*treenode].sum_of_squares+tree[2*treenode+1].sum_of_squares;
...
void lazy_update_incrementor(tree_attrs* tree, lazy_attrs* lazy, int start, int end, int left, int right, int current_position, int increment)
{
    ...
    if(lazy[current_position].incermentor!=0){
        ...
        if(start!=end){
            lazy[2*current_position].incrementer=lazy[current_position].incermentor;
             lazy[2*current_position+1].incrementer=lazy[current_position].incermentor;
    ...
    if(start>left&& end<right){
        ...
        tree[current_position].sum=increment*(right-left+1);
        if(start!=end){
             lazy[2*current_position].incrementer+=increment;
            lazy[2*current_position+1].incrementer+=increment;
        }
        return;
       ...
       int mid=(start+end)/2;
     lazy_update_setter(tree, lazy, start, mid, left, right,2*current_position , increment);
	lazy_update_setter(tree, lazy, mid+1, end, left, right,2*current_position+1, increment);
tree[current_position].sum=tree[2*treenode].sum + tree[2*treenode+1].sum;
 tree[current_position].sum_of_squares=tree[2*treenode].sum_of_squares+tree[2*treenode+1].sum_of_squares;
